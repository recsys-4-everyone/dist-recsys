...
class Metrics:
    def __init__(self, spark, ground_truth_table, i2i_table,
                 user_col="user", item_col="item", relevance_col="relevance", timestamp_col="timestamp",
    ...
    def _label_predictions(self):
        ...
        def _rearrange(record):
            _item1, ((_user, _relevance, _timestamp), _item2_rating) = record
            if not _item2_rating:
                _item2_rating = []
            return _user, ((_item1, _relevance, _timestamp), _item2_rating)
        ...
        def _single_user_metrics(user_recs):
            ...
            recs = []
            for _, user_recs in sort_by_timestamp[:-1]:
                user_recs = sorted(user_recs, key=lambda r: r[1], reverse=True)
                recs.extend([rec[0] for rec in user_recs])
    ...
    def _calc_metrics(ground_truth, relevance, recs):
        ...
        if index >= 0:
            dcg = (math.pow(2, relevance) - 1) / log2(index + 1 + 1)
